 ...
 *
 */
public class AgentToBrowseSQLQueryResultsBonus extends BaseAgent
{
	protected static final Logger log = Logger.getLogger(AgentToBrowseSQLQueryResultsBonus.class);
	...
	protected void execute()
	{
		...
		try
		{
			...
			// We get JDBC reference name from configuration
			/*
			 * We add in our agent an XML configuration like this one :
			 * <header referenceName="myRef" />
			 */
			String referenceName = getConfiguration().getAttribute("referenceName");
			// We get the JDBC Reference
			IJdbcReference iJdbcReference = iWorkflowModule.getJdbcReference(iContext, referenceName);
