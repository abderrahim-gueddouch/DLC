...
public class HideButtonClient extends BaseDocumentExtension {
	...
	public boolean onAfterLoad() {
      ...
      for (IWidget iWidget : widgets)
      {
            ...
            {   
            	if(getWorkflowInstance().getValue("NomDEtape4") == null || getWorkflowInstance().getValue("Desactiver") != null){
            		button.setHidden(true); 
            	}
            }
            if(button.getName().equals("Envoyer") || button.getName().equals("Retour") || button.getName().equals("Valider"))
            {   
            	if(getWorkflowInstance().getValue("NomDEtape4") != null){
            		button.setHidden(true); 
            	}
            }
            if(button.getName().equals("Envoyer") || button.getName().equals("Retour") || button.getName().equals("Valider")){
